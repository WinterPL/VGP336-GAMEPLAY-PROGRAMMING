...
{
	ASSERT(sGraphicsSystem != nullptr, "InputSystem -- No system registered.");
	return sGraphicsSystem.get();
}

GraphicsSystem::~GraphicsSystem()
{
	ASSERT(mD3DDevice == nullptr, "GraphicsSystem must be terminated first to clean up");
}
...
void GraphicsSystem::Initialize(HWND window, bool fullscreen)
{
	...
	HRESULT hr = D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, &featureLevel, 1, D3D11_SDK_VERSION, &swapChainDesc, &mSwapChain, &mD3DDevice, nullptr, &mImmediateContext);
...
void GraphicsSystem::Resize(uint32_t width, uint32_t height)
{
	...
	if (width != GetBackBufferWidth() || height != GetBackBufferHeight())
	{
		...
		ASSERT(SUCCEEDED(hr), "GraphicsSystemfailed to access swap chain view");
	...
	ASSERT(SUCCEEDED(hr), "GraphicsSystem failed to swap chain view");
