...
void ModelIO::LoadMaterial(std::filesystem::path filePath, Model& model)
{
	...
	auto TryReadTeaxtureName = [&](auto& fileName)
	{
	...
	for (auto& materialData : model.materialData)
	{
		...
		TryReadTeaxtureName(materialData.diffuseMapName);
		TryReadTeaxtureName(materialData.specularMapName);
		TryReadTeaxtureName(materialData.bumpMapName);
		TryReadTeaxtureName(materialData.normalMapName);
	}
	fclose(file);
}
